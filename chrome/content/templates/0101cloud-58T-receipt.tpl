...
{/eval}
{eval}
    firstline = _MODIFIERS['substr'](itemTrans.name, 0, 20);
    secondline = itemTrans.name.substr(firstline.length) || '';
{/eval}
[&DHON]${firstline|left:20}${itemTrans.current_qty|right:3}${item.current_subtotal|right:8}[&DHOFF]
{if secondline != ''}
...
{for memo in memos}
--------------------------------
[&DHON]訂單備註：${memo.substr(0,11)}[&DHOFF]
[&DHON]          ${memo.substr(11,11)}[&DHOFF]
[&DHON]          ${memo.substr(22,11)}[&DHOFF] 
{/for}
{/if}
{if customer != null}
--------------------------------
