...
{if order.destination == 'DLV' || order.destination == 'SEND' }
[0x1B][0x72][0x01]
[&QSON]${'DLV:' + order.check_no }  ${header}[&QSOFF]
[0x1B][0x72][0x00]
{else}
[&QSON]${'Table:' + order.table_name}  ${header|right:6}[&QSOFF]
[&QSON]${'Guest:'+ order.no_of_customers}[&QSOFF]
[&QSON]${'Inv#:'+ order.seq}[&QSOFF]
{/if}
[&QSON]${'Time:' + new Date().toLocaleFormat('%H:%M')|left:18}[&QSOFF]
[&QSON]${'Server:'|left:10}${order.service_clerk_displayname|left:16}[&QSOFF]
-------------------------------------
[&CR]
[&QSON]${item.alt_name1}[&QSOFF] [&QSON]${'X'+item.current_qty|right:3}[&QSOFF]
...
{if order.destination == 'DLV' || order.destination == 'SEND'}
[0x1B][0x72][0x01]
[&QSON]${'DLV:' + order.check_no|center} [&QSOFF]
[0x1B][0x72][0x00]
{/if}
[&CR]
[&CR]
