...
[&CR]
{if order.table_name != ''}
[&QSON]${order.destination + '單號' + sequence|left:16}${table_name + '桌'|right:6}[&QSOFF]
[&CR]
{else}
[&QSON]${order.destination|left:12}單號:${sequence|tail:3}[&QSOFF]
[&CR]
{/if}
${'單據號碼:'|left:10}${order.seq|left:13} ${'服務人員:'|left:10}${order.service_clerk_displayname|left:8}
[&DWON]來客數：${order.no_of_customers|left:10}[&DWOFF]
------------------------------------------
{for item in order.display_sequences}
{if item.type == 'item' && order.items[item.index] != null && order.items[item.index].linked}
{eval}
    firstline = _MODIFIERS['substr'](item.name, 0, 28);
    secondline = item.name.substr(firstline.length) || '';
{/eval}
[&DHON]${++index|right:2}${'.'}[&DHOFF][&QSON]${firstline|left:14}[&QSOFF][&DHON]${'x' + order.items[item.index].current_qty|right:4}${item.current_subtotal|right:6}[&DHOFF]
{if secondline != ''}
...
{eval}
    ...
    secondline = item.name.substr(firstline.length) || '';
{/eval}
   [&QSON]→${firstline|left:12}[&QSOFF][&DHON] ${order.items[item.index].current_qty|right:5}[&DHOFF]
{if secondline != ''}
...
電話: ${customer.addr1_tel1|left:36}
...
{/for}
------------------------------------------
${'列印時間:'|left:10}${(new Date()).toLocaleFormat('%m-%d %H:%M:%S')}${'*0207D'|right:6}[&DHON]  總計:${order.item_subtotal|left:7}[&DHOFF]
[&CR]
[&CR]
[&CR]
[&OPENDRAWER]
