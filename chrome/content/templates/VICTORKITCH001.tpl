...
[0x1B][0x72][0x00]
{else}
[&QSON]${'台:' + order.table_name} ${'單:'+ order.check_no}${header|right:6}[&QSOFF]
{/if}
...
{eval}
  ...
  if (childcondiments != null) {
    conds = GeckoJS.BaseObject.getKeys(childcondiments).join(';');
  }
...
{/eval}
[&QSON]    ${childname|left:10} ${'x 1'|right:3}[&QSOFF]
{if childcondStr != ''}
{for cond in conds}
