...
{/eval}
[&QSON]${store.branch|left:16}[&QSOFF]
{if store.address1 != ''}
${store.address1}
{/if}
...
{if customer != null}
********************************
顧客: ${customer.name}  ${'(' + customer.customer_id + ')'}
電話: ${customer.telephone|left:26}
{if customer.addr1_addr1 != ''}
...
{/if}
********************************
${'內容'|left:20}${'數量'|left:6}${'小計'|right:6}
--------------------------------
...
{else}
${'餘額：'|left:20}${order.remain|right:10}
{/if}
{if memos.length > 0}
{for memo in memos}
--------------------------------
