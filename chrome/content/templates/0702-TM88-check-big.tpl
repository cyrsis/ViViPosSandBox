...
{/if}
{if store.state != ''}
[&DHON]${order.destination}單號:${sequence|tail:2}[&DHOFF]
[&CR]
{else}
[&DHON]${order.destination}單號:${sequence|tail:3}[&DHOFF]
[&CR]
{/if}
...
{eval}
	...
	firstline1 = _MODIFIERS['substr'](order.items[item.index].alt_name1, 0, 32);
...
{eval}
    ...
    secondline = item.name.substr(firstline.length) || '';
	firstline1 = _MODIFIERS['substr'](order.items[item.index].alt_name1, 0, 31);
    secondline1 = order.items[item.index].alt_name1.substr(firstline1.length) || '';
{/eval}
...
{/eval}
    [&QSON]${firstline|left:18}[&QSOFF]
	
{if secondline != ''}
    [&QSON]${secondline|left:18}[&QSOFF]
...
{/for}
------------------------------------------
${'列印時間:'|left:10}${(new Date()).toLocaleFormat('%Y-%m-%d %H:%M:%S')}${'*0702'|right:12}
[&CR]
[&CR]
[&CR]
[&OPENDRAWER]
