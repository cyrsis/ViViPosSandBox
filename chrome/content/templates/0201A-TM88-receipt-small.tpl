...
交易序號: ${order.seq|left:26}${'*0201A'|right:9}
----------------------------------------------
...
{/eval}
${itemTrans.name|left:27}${itemTrans.current_qty|right:6}${item.current_subtotal|right:11}
{elseif item.type == 'discount' || item.type == 'surcharge'}
  ◎${item.name|left:30}${item.current_subtotal|right:10}
{elseif item.type == 'setitem'}
${'→'}${order.items[item.index].name|left:26}${item.current_qty|right:6}
