...
------------------------------------------
${'商品名称'|left:28} ${'数量'|left:6} ${'金额'|right:4}
------------------------------------------
...
{/eval}
[&DHON]${itemTrans.name|left:27}${itemTrans.current_qty|right:6}${item.current_subtotal|right:7}[&DHOFF]
{elseif item.type == 'discount' || item.type == 'surcharge'}
  ◎${item.name|left:30}${item.current_subtotal|right:6}
{elseif item.type == 'setitem'}
[&DHON]${'→'}${order.items[item.index].name|left:26}[&DHOFF]${item.current_qty|right:3}
...
总杯数: ${count} 杯
------------------------------------------
小计:       ${order.item_subtotal|right:8}
{if order.discount_subtotal != 0}
折扣金额：  ${order.discount_subtotal|right:8}
{/if}
应收金额:${order.total|right:8}
付款:       ${payment.amount|viviFormatPrices:true|right:8}    {if (0 - order.remain) > 0}找零: ${(0 - order.remain)|right:8}{/if}[&CR]
------------------------------------------
{if store.branch != ''}
店名: ${store.branch|left:30}
{/if}
{if store.telephone1 != ''}
电话: ${store.telephone1|left:30}
{/if}
{if store.address1 != ''}
地址: ${store.address1}
{/if}
...
{if customer.addr1_addr2 != null && customer.addr1_addr2 != ''}
              : ${customer.addr1_addr2|left:32}
{/if}
{if customer.addr1_city != null && customer.addr1_city != ''}
    : ${customer.addr1_city|left:32}
{/if}
{/if}
{if store.country != ''}
