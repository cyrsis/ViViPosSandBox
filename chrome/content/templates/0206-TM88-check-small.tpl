...
{/eval}
[&DHON]${++index|right:2}${'.'}${firstline|left:32}x${order.items[item.index].current_qty|right:4}[&DHOFF]
{if secondline != ''}
   ...[&DHON]${secondline|left:29}[&DHOFF]
{/if}
...
{eval}
    ...
    secondline = item.name.substr(firstline.length) || '';
{/eval}
   [&DHON]→${firstline|left:30}[&DHOFF] ${order.items[item.index].current_qty|right:4}
{if secondline != ''}
   ...[&DHON]${secondline|left:29}[&DHOFF]
{/if}
...
------------------------------------------
${'列印時間:'|left:10}${(new Date()).toLocaleFormat('%m-%d  %H:%M:%S')}${'*0206'|right:14}
[&CR]
[&CR]
[&CR]
[&OPENDRAWER]
