...
交易序號: ${order.seq|left:26}${'*0201'|right:9}
----------------------------------------------
...
{/eval}
{eval}
    firstline = _MODIFIERS['substr'](itemTrans.name, 0, 27);
    secondline = itemTrans.name.substr(firstline.length) || '';
{/eval}
[&DHON]${firstline|left:27}${itemTrans.current_qty|right:6}${item.current_subtotal|right:11}[&DHOFF]
{if secondline != ''}
