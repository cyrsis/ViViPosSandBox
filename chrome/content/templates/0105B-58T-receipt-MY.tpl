...
{/eval}
${itemTrans.name|left:32}${'x ' + itemTrans.current_qty|right:23}${item.current_subtotal|right:9}
...
{eval}
  if (order.items[item.index].parent_index) {
    condimentName = '' + item.name + '(' + item.current_qty + ')';
...
subtotal:         ${'RM,' + order.item_subtotal|right:13}
{if order.discount_subtotal != 0}
discount:         ${'RM,' + order.discount_subtotal|right:13}
{/if}
{if order.surcharge_subtotal != 0}
surcharge:        ${'RM,' + order.surcharge_subtotal|right:13}
{/if}
{if order.item_subtotal - order.total + order.discount_subtotal + order.surcharge_subtotal > 0}
promotion:        ${'RM,' + (-(order.item_subtotal - order.total + order.discount_subtotal + order.surcharge_subtotal))|right:13}
{/if}
--------------------------------
{/if}
${'Amount:'|left:18} ${'RM,' + txn.formatPrice(order.total)|right:12}
--------------------------------
${'**** payment ****'|center:32}
{for payment_type in payment_types}
${payment_type.display_name |left:20}${'RM,' + txn.formatPrice(payment_type.amount)|right:11}
{/for}
{if 0 - order.remain > 0}
change:           ${'RM,' + txn.formatPrice(0 - order.remain)|right:13}
{/if}
{if memos.length > 0}
{for memo in memos}
