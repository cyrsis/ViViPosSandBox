...
[0x1C][0x70][0x01][0x00]
  [&DWON]${"The Red Gate"|center:20}[&DWOFF]
             Korean Fine Cusine
{if duplicate}
...
{if item.type == 'item'}
${item.current_qty|right:2} ${item.name|left:25} ${item.current_price|right:5}   ${item.current_subtotal}
...
${'Subtotal:'|left:15} ${(order.item_subtotal)|viviFormatPrices:true|right:24}
[&CR]
${'Payment:'|left:15}
