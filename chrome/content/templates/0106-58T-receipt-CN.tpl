...
{if store.state != ''}
[&QSON]${order.destination}单号:${sequence|tail:2}[&QSOFF]
{else}
[&QSON]${order.destination}单号:${sequence|tail:3}[&QSOFF]
{/if}
{if order.check_no != ''}
[&QSON]牌号: ${order.check_no}[&QSOFF]
{/if}
{if order.table_name != ''}
[&QSON]桌名:${table_name|left:10}[&QSOFF]
{/if}
日期:${today|left:12}时间:${time}
交易序号: ${order.seq|left:13}    *0106
--------------------------------
${'商品名称'|left:20}${'数量'|left:6}${'価格'|right:6}
--------------------------------
...
原始总金额：         ${order.item_subtotal|right:10}
{if order.discount_subtotal != 0}
折扣总金额：         ${order.discount_subtotal|right:10}
{/if}
{if order.surcharge_subtotal != 0}
服务费金额：         ${order.surcharge_subtotal|right:10}
{/if}
{if order.item_subtotal - order.total + order.discount_subtotal + order.surcharge_subtotal > 0}
促销总金额：         ${-(order.item_subtotal - order.total + order.discount_subtotal + order.surcharge_subtotal)|right:10}
{/if}
--------------------------------
{/if}
[&DHON]${'总计:'|left:20}${count|right:3}${order.total|right:6}元[&DHOFF]
--------------------------------
...
{for memo in memos}
--------------------------------
[&DHON]订单备注：${memo|left:21}[&DHOFF]
{/for}
{/if}
{if customer != null}
--------------------------------
名称: ${customer.name|left:26}
电话: ${customer.telephone|left:26}
