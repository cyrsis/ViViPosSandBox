...
[0x1C][0x70][0x01][0x00]
[&QSON]       MAIZURU[&QSOFF]
[&QSON]  Japanese Restaurant[&QSOFF]
{if duplicate}
...
[&QSON]${'檯號:'+order.table_name|right:14}[&QSOFF]

數量   食品                       銀碼
-----------------------------------------
{for item in order.display_sequences}
{if item.type == 'item'}
${item.current_qty|right:4} ${item.name|left:24}${item.current_subtotal|right:8}
...
${'小計:'|left:15}${(order.total)|viviFormatPrices:true|right:22}
[&CR]
${'付款方式:'|left:15}
...
{/if}
{/if}
[&QSON]${'應付金額:'|left:5}       ${order.total|viviFormatPrices:true}[&QSOFF]
