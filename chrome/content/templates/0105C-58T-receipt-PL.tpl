...
subtotal:         ${'P,' + order.item_subtotal|right:13}
{if order.discount_subtotal != 0}
discount:         ${'P,' + order.discount_subtotal|right:13}
{/if}
{if order.surcharge_subtotal != 0}
surcharge:        ${'P,' + order.surcharge_subtotal|right:13}
{/if}
{if order.item_subtotal - order.total + order.discount_subtotal + order.surcharge_subtotal > 0}
promotion:        ${'P,' + (-(order.item_subtotal - order.total + order.discount_subtotal + order.surcharge_subtotal))|right:13}
{/if}
--------------------------------
{/if}
${'Amount:'|left:18} ${'P,' + txn.formatPrice(order.total)|right:12}
--------------------------------
${'**** payment ****'|center:32}
{for payment_type in payment_types}
${payment_type.display_name |left:20}${'P,' + txn.formatPrice(payment_type.amount)|right:11}
{/for}
{if 0 - order.remain > 0}
change:           ${'P,' + txn.formatPrice(0 - order.remain)|right:13}
{/if}
{if memos.length > 0}
{for memo in memos}
